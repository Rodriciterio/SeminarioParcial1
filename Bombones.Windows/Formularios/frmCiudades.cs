...
namespace Bombones.Windows
{
    ...
    public partial class frmCiudades : Form
    {
        ...
        private void ascendenteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                var listaOrdenada = ServiciosCiudades.GetListaOrdenadaPorCiudad(true);
                MostrarDatosEnGrilla(listaOrdenada);
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error al ordenar las ciudades: {ex.Message}", "Error",
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
